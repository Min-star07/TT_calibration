...
std::vector<double> GaussFitter::FitPedestal(TH1F* hist) {
    ...
    double x_left = hist->GetXaxis()->GetBinCenter(maxBin) - 3.0;  // Define a range around the peak
    double x_right = hist->GetXaxis()->GetBinCenter(maxBin) + 3.0;
    ...
    hist->Fit(gaussFunc, "RQ"); // "R" to restrict the range, "Q" for quiet mode to suppress printouts
