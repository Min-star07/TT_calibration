...
std::vector<double> GaussFitter::FitPedestal(TH1F* hist) {
    ...
    double x_left = hist->GetXaxis()->GetBinCenter(maxBin) - 1.0; // Define a range around the peak
    double x_right = hist->GetXaxis()->GetBinCenter(maxBin) + 1.0;
...
void GaussFitter::SaveHistogramToRootFile(TH1F* hist, TF1* func, const std::string& newFilename, const std::string& histName) {
    ...
    outFile->cd();
    hist->GetXaxis()->SetRangeUser(15, 30);
