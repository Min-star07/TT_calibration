...
class Plotter:
    ...
    def draw_scatter(self, x, chi2perndf, y, yerr, pdf_pages):
        ...
        median_value = chi2perndf.median()
        # Top plot: Scatter plot with error bars
        ax1.errorbar(
            x,
            y,
            yerr=yerr,
        ...
        # plt.errorbar(x, y, yerr=yerr, capsize=5, color="skyblue", edgecolor="black")
    ...
    def Q1_distri_hist2d(self, filepath, x, y):
        ...
        for i in range(data.shape[0]):
            for j in range(data.shape[1]):
                plt.text(
                    j, i, f"{data[i, j]:.1f}", ha="center", va="center", color="red"
                )
