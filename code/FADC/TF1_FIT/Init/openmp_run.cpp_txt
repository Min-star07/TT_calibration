...
//             std::string command = "./Run_Fit -ROB " + std::to_string(ROB_num[ROB]) 
//                                   + " -CB " + std::to_string(CB_num[0]) 
//                                   + " -fl " + fl 
//                                   + " -type " + type
//                                   + " -v " + std::to_string(HV_num[0]);
...
int main(int argc, char **argv) {
    ...
    // List of ROB values to loop over
    const int ROB_values[] = {5, 15};
    #pragma omp parallel for
    for (int i = 0; i < 2; i++) {
        int ROB = ROB_values[i];

        // Build the command with the full file name and type
        string command = "./Run_Fit -ROB " + to_string(ROB) 
                        + " -CB " + to_string(CB) 
                        + " -fl " + file_led_default
                        + " -type " + type_default
                        + " -v " + to_string(HV);
        ...
        // Print the command and the thread number
        cout << "\n Run command: " << command 
             << " in thread " << omp_get_thread_num() << endl;
...
//  g++ -fopenmp openmp_run.cpp -o openmp_run